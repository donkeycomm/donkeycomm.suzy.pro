...
const RegisterContact: FC<registercontactProps> = ({
  ...
  const registerContact = async () => {
    ...
    //check if email is valid and not empty and also check if selectedContactLists is not empty
    if (email.trim() === '' || selectedContactLists.length === 0) {
      setError('E-mail and contact list are required');
      setLoading(false);
    } else {
      if (storedJWT) {
        await axios
  ...
  const selectListItem = (item: any) => {
    if (!selectedContactLists.includes(item)) {
      ...
      setContactsLists(contactLists.filter((list: any) => list.ID !== item.ID));
    }
  ...
  return (
    <div className="inline-block w-full p-6 pt-6 pr-6 mt-10 bg-white rounded-lg shadow-md fade-in md:mt-5">
      <div className="flex justify-between mb-3">
        <h1 className="mb-4 text-xl text-default">Add a contact</h1>
        <svg
